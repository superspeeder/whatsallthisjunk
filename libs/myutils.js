...
/**
 * @template T 
 * @param {DistributionDefEntry<T>[]} distribution
 */
 ...
 * @returns {T[]}
 */
function generateWeightedCollection(distribution, totalCount) {
    // We need to be using a normalized distribution for this function.
    let distributionReal = normalizeDistribution(distribution);
    ...
    if (totalGenerated != totalCount) { // Everything didn't divide evenly, add some randomly weighted by the distribution.
        for (let i = 0; i < totalCount - totalGenerated; i++) {
            results.push(weightedChoice(distributionReal));
        }
    }
